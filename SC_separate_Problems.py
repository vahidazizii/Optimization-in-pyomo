...
workbook = xlsxwriter.Workbook('Result_separate_small.xlsx')
...
for cc in range(1,11):
    ...
    def setupproblem():
        ...
        # Assignment(i)$(ord(i)>nc)..sum((j)$(ord(j)<(nc+1)),z(i,j))=e=1;
        ...
        for i in range(len(del_set)):
            ...
            problem.linear_constraints.add(
                lin_expr=[cplex.SparsePair(zvars, zcoefs)],
                senses=["E"],
                rhs=[1])
        ...
        for i in range(len(del_set)):
            ...
            problem.linear_constraints.add(
                lin_expr=[cplex.SparsePair(thevars, thecoefs)],
                senses=["E"],
                rhs=[1], names=["routing4_{0}".format(i + 1)])
        ...
        global LRPOBJ
        LRPOBJ=problem.solution.get_objective_value()
        # endregion
    
        # region Results
        # print obj value
        print("Optimal LRP cost=" + str(Optimal_Cost))
        ...
        global z_fix
        global u_fix
        x_fix=solution[0:x_bound]
        w_fix=solution[x_bound:w_bound]
        z_fix=solution[w_bound:z_bound]
        u_fix=solution[z_bound:u_bound]
    ...
    # Defining problem routing problem for suppliers and direct shipment problems
    def setupproblem2():
    
        # region Adding variables
        problem2= cplex.Cplex()
        problem2.objective.set_sense(problem2.objective.sense.minimize)
            ...
            for j in cros_del_set:
                ...
                for v in range(1, num_del_v[0] + 1):
                    ...
                    if i > num_cr[0] and j > num_cr[0]:
                        same_product = 0
                        for r in range(len(prod_set)):
                            same_product += float(float(b.values[i - len(cros_set) - 1][r])) * float(
                                float(b.values[j - len(cros_set) - 1][r]))
        ...
        xSET = list(problem2.variables.add(names=allxvars, lb=x_fix,
                                          ub=x_fix,
                                          types=["B"] * len(allxvars), obj=xcoef))
        ...
        wSET = list(problem2.variables.add(names=allwvars, lb=w_fix,
                                          ub=w_fix,
                                          types=["B"] * len(allwvars), obj=wcoef))
        ...
        zSET = list(problem2.variables.add(names=allzvars, lb=z_fix,
                                          ub=z_fix,
                                          types=["B"] * len(allzvars)))
            ...
            for j in cros_del_set:
                ...
                for v in range(1, num_del_v[0] + 1):
                    ...
                    if i < (float(num_cr[0]) + 1) and j < (float(num_cr[0]) + 1):
                        ...
                        upperbound.append(float(cap_v[0]))
        uSET = list(problem2.variables.add(names=alluvars, lb=u_fix,
                                          ub=u_fix,
                                          types=["C"] * len(alluvars), obj=ucoef))
        ...
        DSET = list(problem2.variables.add(names=allDvars, lb=[0] * len(allDvars),
                                          ub=[1] * len(allDvars),
                                          types=["B"] * len(allDvars), obj=Dcoef))
        ...
        ySET = list(problem2.variables.add(names=allyvars, lb=[0] * len(allyvars),
                                          ub=upper_bound,
                                          types=["B"] * len(allyvars), obj=ycoef))
        ...
        sSET = list(problem2.variables.add(names=allsvars, lb=[0] * len(allsvars),
        ...
        for i in range(len(cros_set)):
            ...
            for p in range(len(cros_sup_set)):
                ...
                for r in range(len(prod_set)):
                    ...
                    for v in range(1, num_sup_v[0] + 1):
                        varname = "g(" + str(cros_set[i]) + "," + str(cros_sup_set[p]) + "," + str(
                            prod_set[r]) + "," + str(
                            v) + ")"
                        allgvars.append(varname)
                        if p < num_cr[0]:
        ...
        gSET = list(problem2.variables.add(names=allgvars, lb=[0] * len(allgvars),
                                          ub=ub,
                                          types=["C"] * len(allgvars)))
        # endregion
        ...
        problem2.parameters.parallel.set(-1)  # opportunistic parallel search mode
        #problem2.write("SupplyChain.lp")
        problem2.parameters.threads.set(16)
    
        start_time2=time.time()
        problem2.solve()
        elapsed_time2 = time.time() - start_time2
    
        sol = problem2.solution
        solution = sol.get_values()
        OverallObj=sol.get_objective_value()
    
        # endregion
    
        # region Results
        # print obj value
        print("Optimal cost=" + str(OverallObj))
        # print variables values
        x_bound = (len(del_set) + len(cros_set)) * (len(del_set) + len(cros_set)) * num_del_v[0]
        w_bound = x_bound + len(cros_set)
        z_bound = w_bound + (len(del_set) * len(cros_set))
        ...
        worksheet.write(cc, 1, OverallObj)
        elapsed_time_total = elapsed_time2 + elapsed_time
        worksheet.write(cc, 2, elapsed_time_total)
