...
lb=1
lb=2
Writefilename="Result_Inegrated_Medium"+str(lb)+".xlsx"

workbook = xlsxwriter.Workbook(Writefilename)
...
for cc in range(lb,ub):
    ...
    cdr = pd.read_excel(filename, sheetname='cdr',
                        header=None)  # cost of direct supply from supplier su to customer cu >>cost of arcs between suppliers and customers
    cd = pd.read_excel(filename, sheetname='cd',
                       header=None)  # distance between node i to node j >> distances among customers and crossdocks
    ...
    cp = pd.read_excel(filename, sheetname='cp',
                       header=None)  # distance between node p to node t >>distances among suppliers and crossdocks
    b = pd.read_excel(filename, sheetname='b',
                      header=None)  # identifier of type of demanded good(r) by customer j
    a = pd.read_excel(filename, sheetname='a',
                      header=None)  # identifier of type of produced good(r) by supplier p
