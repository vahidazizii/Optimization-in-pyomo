...
workbook = xlsxwriter.Workbook('Result_Mixed_Demand_Delivery_small.xlsx')
...
#for loop over all instances
for cc in range(1,11):
    print(cc)
    filename="Small "+str(cc)+".xlsx"
    print(filename)

    # region DEFINING PARAMETERS
    num_sup = pd.read_excel(filename, sheetname='supp_num', header=None)  # number of suppliers
    num_cr= pd.read_excel('supply_chain.xlsx', sheetname='cross_num', header=None)       #number of crossdocks
    num_del = pd.read_excel('supply_chain.xlsx', sheetname='del_num', header=None)  # number of delivery customers
    num_prod = pd.read_excel('supply_chain.xlsx', sheetname='prod_num', header=None)  # number of products
    num_del_v = pd.read_excel('supply_chain.xlsx', sheetname='num_del_v', header=None)  # number of delivery vehicles
    num_sup_v = pd.read_excel('supply_chain.xlsx', sheetname='num_sup_v', header=None)  # number of pickup vehicles
    cap_v = pd.read_excel('supply_chain.xlsx', sheetname='cap_v', header=None)  # capacity of delivery vehicles
    cap_vp = pd.read_excel('supply_chain.xlsx', sheetname='cap_vp', header=None)  # capacity of pickup vehicles
    FV_p = pd.read_excel('supply_chain.xlsx', sheetname='FV_p', header=None)  # fixed cost of pickup vehicles
    FV_d = pd.read_excel('supply_chain.xlsx', sheetname='FV_d', header=None)  # fixed cost of delivery vehicles
    FV_Dr = pd.read_excel('supply_chain.xlsx', sheetname='FV_Dr', header=None)  # fixed cost of direst shipment vehicle
    cap_c = pd.read_excel('supply_chain.xlsx', sheetname='cap_c', header=None)  # Capacity of crossdock c
    cap_s = pd.read_excel('supply_chain.xlsx', sheetname='cap_s', header=None)  # Maximum amount of supply by supplier p
    cdr = pd.read_excel('supply_chain.xlsx', sheetname='cdr',header=None)  # cost of direct supply from supplier su to customer cu >>cost of arcs between suppliers and customers
    cd = pd.read_excel('supply_chain.xlsx', sheetname='cd', header=None)  # distance between node i to node j >> distances among customers and crossdocks
    FW = pd.read_excel('supply_chain.xlsx', sheetname='FW', header=None)  # Openning cost of crossdock c
    dem = pd.read_excel('supply_chain.xlsx', sheetname='dem', header=None)  # demand of customer
    cp = pd.read_excel('supply_chain.xlsx', sheetname='cp',header=None)  # distance between node p to node t >>distances among suppliers and crossdocks
    b = pd.read_excel('supply_chain.xlsx', sheetname='b', header=None)  # identifier of type of demanded good(r) by customer j
    a = pd.read_excel('supply_chain.xlsx', sheetname='a',header=None)  # identifier of type of produced good(r) by supplier p
        ...
        for i in cros_del_set:
            ...
            for j in cros_del_set:
                ...
                for v in range(1, num_del_v[0] + 1):
                    ...
                    if (i < (float(num_cr[0]) + 1) and j < (float(num_cr[0]) + 1)) or i == j:
                        upperbound.append(0)
                    else:
                        upperbound.append(1)
